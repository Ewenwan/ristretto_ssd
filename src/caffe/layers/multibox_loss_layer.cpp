...
/*
        if (i == 1 && m == 5202) {
	    LOG(INFO) << "gt_idx : " << match_indices[label][m];
	}
*/
        if (match_indices[label][m] != -1) {
          ++num_pos;
          if (match_indices[label][m] > temp_max) {
		 temp_max = match_indices[label][m];
		 if (i == 1 && m == 5202) {
			LOG(INFO) << "Print label : " << match_indices[label][m];
		 }
	  }
        }
      }
      //LOG(INFO) << "Maximum ground truth label : " << temp_max;
    ...
    all_match_indices_.push_back(match_indices);
    if (all_match_indices_.size() > 1) {
	map<int, vector<int> > v = all_match_indices_[1];
	//int label_ch = v.first;
	vector<int>& match_index_ch = v[-1];
	int gt_idx_ch = -1;
	for (int j = 0; j < match_index_ch.size(); j++) {
		if (match_index_ch[j] != -1 && match_index_ch[j] > gt_idx_ch) {
			gt_idx_ch = match_index_ch[j];
		}
	}		
	//LOG(INFO) << "iteration number : " << i;
	//LOG(INFO) << "MAX gt_idx_ch : " << gt_idx_ch;
    }
    all_neg_indices_.push_back(neg_indices);
  }
